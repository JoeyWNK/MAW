...
public class AutoBattle {
	...
	static double HpCombo(Deck d)
	{
		double ans=1;
			 if(d.num==6)ans+=0.2;
		else if(d.num==9)ans+=0.3;
		else if(d.num==12)ans+=0.4;
		return ans;
	}
	...
	static int Battle(Deck mydeck,int fairyhp,int fairyatk,int EX)
	{
		...
		//System.out.println(pp[0]+" "+pp[1]+" "+pp[2]);
			...
			p[row][this_round][0]=p[row][last_round][0]*(1-pp[0])*(1-pp[1])*(1-pp[2]);
			
			p[row][this_round][1]=p[row][last_round][1]*(1-pp[1])*(1-pp[2])+p[row][last_round][0]*pp[0];
			p[row][this_round][2]=p[row][last_round][2]*(1-pp[0])*(1-pp[2])+p[row][last_round][0]*(1-pp[0])*pp[1];
			p[row][this_round][4]=p[row][last_round][4]*(1-pp[0])*(1-pp[1])+p[row][last_round][0]*(1-pp[0])*(1-pp[1])*pp[2];
			
			p[row][this_round][3]=p[row][last_round][3]*(1-pp[2])+p[row][last_round][1]*pp[1]+p[row][last_round][2]*pp[0];
			p[row][this_round][5]=p[row][last_round][5]*(1-pp[1])+p[row][last_round][1]*(1-pp[1])*pp[2]+p[row][last_round][4]*pp[0];
			p[row][this_round][6]=p[row][last_round][6]*(1-pp[0])+p[row][last_round][2]*(1-pp[0])*pp[2]+p[row][last_round][4]*(1-pp[0])*pp[1];
			
			p[row][this_round][7]=p[row][last_round][7]+p[row][last_round][3]*pp[2]+p[row][last_round][5]*pp[1]+p[row][last_round][6]*pp[0];

			p[row][this_round][8]=p[row][this_round][1]+p[row][this_round][3]+p[row][this_round][5]+p[row][this_round][7];
			p[row][this_round][9]=p[row][this_round][2]+p[row][this_round][3]+p[row][this_round][6]+p[row][this_round][7];
			p[row][this_round][10]=p[row][this_round][4]+p[row][this_round][5]+p[row][this_round][6]+p[row][this_round][7];
			...
			for(int i=0;i<3 && i<mydeck.num;i++){
				...
				{
					case 0:fairyhp-=pc[i]*effect;
						break;
					case 1:recover=(int)min(effect,max_hp-hp);
						   hp+=pc[i]*recover;
						break;
					case 2:fairyhp-=pc[i]*effect;	
						break;
					case 3:fairyhp-=pc[i]*effect/10.0*round;
						break;
					case 4:
	...
	public static String GetWinDeck(int lv,int fairyhp,int remainhp,int fairyatk,int bc,int EX){
	...
	public static String GetCollectionDeck(int lv,int type,int fairyhp,int remainhp,int fairyatk,int bc,double expect,int EX){
		
		int ans=-1;
		int gather=0;
		double k=expect;
		
		if(type==1 || type==3)gather=40*lv+1000;
		else				  gather=10*lv;
