...
public class GetBattleResult {
	...
	public static void getBattleResult(Document doc) throws Exception {
		...
		if (spec.length() == 0) {
			GetGather = Integer.parseInt(xpath.evaluate(
					"//battle_result/special_item/after_count", doc))
					- Integer.parseInt(xpath.evaluate(
							"//battle_result/special_item/before_count", doc));
			...
			GetGather = Integer
					...
					- Integer
							.parseInt(xpath
									.evaluate(
											"//private_fairy_reward_list/special_item/before_count",
											doc));
		}
		if (spec.length() != 0) {
			...
			Process.info.GetGather = GetGather;
		} else {
