...
public class SellCard {
	...
	public static boolean run() throws Exception {
		...
		try {
			 ...
			 * ErrorData.currentErrorType =
			 * ErrorData.ErrorType.SellCardDataError; ErrorData.bytes = response;
			 **/
			throw ex;
		}
		...
		String errorMessage = xpath.evaluate("/response/header/error/message",
				doc);
		try {
			if (!xpath.evaluate("/response/header/error/code", doc).equals(
				...
				/**
				 * ErrorData.currentErrorType =
				 * ErrorData.ErrorType.SellCardResponse;
				 * ErrorData.currentDataType = ErrorData.DataType.text;
				 **/
				Go.log(errorMessage);
				return false;
				...
				// ErrorData.text =
				// xpath.evaluate("/response/header/error/message", doc);
				// Process.info.toSell = "";
				Go.log(errorMessage);
				return true;
			}
			...
			/**
			 * if (ErrorData.currentErrorType != ErrorData.ErrorType.none) throw
			 * ex; ErrorData.currentDataType = ErrorData.DataType.bytes;
			 * ErrorData.currentErrorType =
			 * ErrorData.ErrorType.SellCardDataError; ErrorData.bytes =
			 * response;
			 **/
			throw ex;
		}
