...
public class CryptoCn {
	...
	public static byte[] BaseSecretKey = BaseSecretKeyCN.getBytes(); // cn?tw?
																		// key2
	...
	public static byte[] decode64(byte[] paramArrayOfByte) {
		SecretKeySpec localSecretKeySpec = new SecretKeySpec(BaseSecretKey,
				"AES");
		try {
	...
	public static String encodeWithBase64(String paramString) {
		...
		try {
			...
			Cipher localCipher = Cipher.getInstance("AES/ECB/PKCS5Padding");
			localCipher.init(Cipher.ENCRYPT_MODE, localSecretKeySpec);
			String str = Base64.encodeBase64String(localCipher
					.doFinal(arrayOfByte));
			return str;
		} catch (Exception localException) {
		}
	...
	public static void setKey(String paramString) {
		System.out.println(String.format("> set K C:%s",
	...
	public List<NameValuePair> addnew_crypt_K_param(List<NameValuePair> param,
			String url) {
		...
		while (i.hasNext()) {
			...
			String val = Base64.encodeBase64String(new_encrypt_cn(E_CODE.AES,
					n.getValue()));
			if (is_login == true) {
				localArrayList
						.add(new BasicNameValuePair(n.getName(), Base64
								.encodeBase64String(new_encrypt_cn(
										E_CODE.RSA_EP, val))));
			} else {
	...
	public void generate_new_aes_key() {
		...
		try {
			...
			aes_key = Base64.encodeBase64String(localKeyGenerator.generateKey()
					.getEncoded());
		...
		try {
			if (paramE_CODE == E_CODE.RSA) {
				...
				SecretKeySpec localSecretKeySpec = new SecretKeySpec(
						Base64.decodeBase64(aes_key.getBytes()), "AES");
